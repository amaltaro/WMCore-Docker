...
cleanup_oracle()
{
  echo "AMR for Docker: skipping it"
  return 0
...
### Assume it's an image for CERN nodes
...
for step in prep sw post; do
  ...
  if [ $? -ne 0 ]; then
    find $BASE_DIR -name \*.log
    #cat $BASE_DIR/wmagent/.deploy/*-${step}.log
    cat /data/srv/wmagent/v1.1.15.patch5/sw/bootstrap-slc7_amd64_gcc630.log
    exit 1111
fi
done
#set +e
...
cd $MANAGE
echo "AMR for Docker: not populating resource-control"
...
### FIXME: when image is built, we don't have a valid certificate, so this query will fail
...
echo "AMR for Docker: not inserting/updating agent configuration"
